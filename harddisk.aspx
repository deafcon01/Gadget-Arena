<%@ Page Language="C#" AutoEventWireup="true" CodeFile="harddisk.aspx.cs" Inherits="harddisk" %>
...
<body>
    ...
    <div>
        ...
        <asp:Label ID="Label1" runat="server" Text="Hard Disk" CssClass="style1"></asp:Label>
        <br />
        <asp:SqlDataSource ID="SqlDataSource1" runat="server" 
            ConflictDetection="CompareAllValues" 
            ConnectionString="<%$ ConnectionStrings:ConnectionString %>" 
            DeleteCommand="DELETE FROM [Product] WHERE [pid] = @original_pid AND [category] = @original_category AND [brand_name] = @original_brand_name AND [prod_name] = @original_prod_name AND [price] = @original_price AND [size] = @original_size AND [weight] = @original_weight AND [img_url] = @original_img_url" 
            InsertCommand="INSERT INTO [Product] ([category], [brand_name], [prod_name], [price], [size], [weight], [img_url]) VALUES (@category, @brand_name, @prod_name, @price, @size, @weight, @img_url)" 
            OldValuesParameterFormatString="original_{0}" 
            SelectCommand="SELECT [category], [pid], [brand_name], [prod_name], [price], [size], [weight], [img_url] FROM [Product] WHERE ([category] = @category)" 
            UpdateCommand="UPDATE [Product] SET [category] = @category, [brand_name] = @brand_name, [prod_name] = @prod_name, [price] = @price, [size] = @size, [weight] = @weight, [img_url] = @img_url WHERE [pid] = @original_pid AND [category] = @original_category AND [brand_name] = @original_brand_name AND [prod_name] = @original_prod_name AND [price] = @original_price AND [size] = @original_size AND [weight] = @original_weight AND [img_url] = @original_img_url">
            <DeleteParameters>
        ...
        <asp:GridView ID="GridView1" runat="server" AutoGenerateColumns="False" 
            ...
            GridLines="Horizontal" CssClass="style2" Height="437px" Width="613px">
